...
Rico.LiveGrid.prototype = {
  ...
  initialize: function( tableId, buffer, options ) {
    ...
    Object.extend(this.options, {
      ...
      // other images
      shadowImg           : 'shadow.png',
      shadowllImg         : 'shadow_ll.png',
      shadowurImg         : 'shadow_ur.png',
      
      // bookmark
      bookmarkText        : null,
      bookmarkTextLoading : null,
      bookmarkTextNoMR    : null,
      bookmarkTextNoR     : null
    });
    // other options:
    ...
    if (this.options.prefetchBuffer) {
      if (this.bookmark) {
        /* * Alex Botez */
        if (null != this.options.bookmarkTextLoading) {
            this.bookmark.innerHTML = this.options.bookmarkTextLoading;
        }
        else {        
            this.bookmark.innerHTML = RicoTranslate.getPhraseById('bookmarkLoading');
        }
      }
 ...
 */
  ...
  sortHandler: function() {
    ...
    this.clearRows();
    /* * Alex Botez */
    if (this.buffer.lastOffset && !isNaN(this.buffer.lastOffset)) {
        this.buffer.fetch(this.buffer.lastOffset);
    }
    else {
        this.scrollDiv.scrollTop = 0;
        this.buffer.fetch(0);
  ...
  bookmarkHandler: function(firstrow,lastrow) {
    ...
    if (totrows<=0) {
      /* * Alex Botez */
      if (null != this.options.bookmarkTextNoMR) {        
        var newhtml = this.options.bookmarkTextNoMR;
      }
      else {
      ...
      /* * Alex Botez */
      if (null != this.options.bookmarkTextNoMR) {
        var newhtml = this.options.bookmarkTextNoR;
      }
      else {
        var newhtml = RicoTranslate.getPhraseById('bookmarkNoRec');
      }
    } else if (this.buffer.foundRowCount) {
      if (null != this.options.bookmarkText) {
        var newhtml = this.options.bookmarkText.format(firstrow, lastrow, totrows);
      }
      else {
